...
describe Newpoc::Action::Post::Show do
  let(:post_repo) do
    Class.new do
  ...
  it 'has a version number' do
    expect(Newpoc::Action::Post::Show::VERSION).not_to be nil
  end

  context 'with default option settings' do
    let(:command) do
      described_class.new target_slug, current_user, post_repo
    end
  ...
  context 'with non-default option settings' do
    let(:command) do
      described_class.new target_slug, current_user, post_repo, options
    end
